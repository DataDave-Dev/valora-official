...
/**
 * Página dedicada para crear (`/metas/nueva`) o editar (`/metas/:id/editar`)
 * una meta. No es un modal: envuelve `MetaForm` y resuelve la navegación.
 */
export function MetaFormPage() {
  ...
  useEffect(() => {
    ...
    if (id && metas.length === 0) void fetchMetas(hogarId)
  }, [hogarId, id, cuentas.length, metas.length, fetchCuentas, fetchMetas])
  ...
  return (
    <div>
      <header className="mb-6 flex items-center gap-3">
        ...
        <h1 className="text-headline-sm text-on-surface">{esEdicion ? 'Editar meta' : 'Nueva meta'}</h1>
      </header>
      ...
      <div className="mx-auto max-w-2xl">
        <div className="rounded-2xl border border-outline-variant bg-surface-container-lowest p-6 shadow-sm">
          {esEdicion && !meta ? (
            ...
            <MetaForm meta={meta} onSuccess={volver} onCancel={() => navigate(-1)} />
          )}
        </div>
      </div>
    </div>
